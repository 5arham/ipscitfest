...
import {
  CONFIG, NAV, EVENTS, RULES, SCHEDULE, SCHEDULE_STATUS, SCHEDULE_CONFLICTS,
...
function bar(slug) {
  ...
  return `
    <header class="bar">
      <a class="bar-mark" href="index.html">IPSC IT Fest <span>26</span></a>
...
const NAV_BLURB = {
  ...
  about: 'What the fest is, who may enter, and the standard every event pack is written to.',
  events: 'All eight events, their rules, judging and participant counts.',
  schedule: 'The provisional running order across setup, arrivals and three days of competition.',
  ...
  resources: 'Brochure, invitation, forms, and the documentation still to come.',
  committee: 'The masters and students running the fest, and every event head.',
  chandbagh: 'The campus, the four working spaces, and how to reach Dehradun.',
...
function shell({ slug, title, description, body, depth = 0, bodyClass = '', nav = true }) {
  ...
  const barHtml = bar(slug).replace(/href="(?!http|mailto|#)/g, `href="${up}`);
  const stripHtml = strip().replace(/href="(?!http|mailto|#)/g, `href="${up}`);
  const footHtml = footer().replace(/href="(?!http|mailto|#)/g, `href="${up}`);
...
function portrait(slug, { up = '', className = '' } = {}) {
  ...
  return `<figure class="mug ${className}">
        <span class="mug-frame">
          ...
          <img src="${up}assets/people/${esc(slug)}.jpg" alt="${esc(name)}" loading="lazy" decoding="async" />
        </span>
      </figure>`;
}
  ...
  return `
        <article class="bio">
          ...
          <div class="bio-text">
            ...
            ${isTbc(p.bio)
              ? `${ruled(9)}<p class="bio-body bio-body--tbc">${val(p.bio)} &mdash; ${esc(plain(p.bio))}</p>`
    ...
    return `
        <div class="staff-head">
          ...
          <div>
            ...
            <p class="staff-bio">${val(p.bio)}${isTbc(p.bio) ? ` &mdash; ${esc(plain(p.bio))}` : ''}</p>
          </div>
        </div>`;
  }).join('');
  ...
  return `
    <section class="band band--tight">
      <div class="list-head">
        ...
        <p class="annot">${EVENT_DOCS(ev).length} documents &middot; released ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}</p>
      </div>
      ...
      <p class="note">Documents are circulated to registered schools in the ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}, and appear here at the same time.</p>
    </section>`;
}
  ...
  switch (ev.sim) {
    case 'slam': return `
      <figure class="fig" data-sim="slam" data-alt="A rover surveying an unmapped arena. Two paths are drawn: its true track and the track it believes it took. They separate as odometry drifts and snap back together when it recognises ground it has already mapped.">
        <figcaption class="fig-head">
          <span class="annot">Coverage traverse &mdash; occupancy grid, live</span>
          <span class="fig-read" id="slamRead">coverage 00%</span>
        </figcaption>
        <div class="fig-body"></div>
        <div class="fig-foot">
          <span class="annot" id="slamStatus">Surveying &mdash; obstacles resolve as they are found</span>
    ...
    case 'astar': return `
      <figure class="fig" data-sim="astar" data-alt="An autonomous ground vehicle planning a route through a corridor network to a coloured bin, with the search frontier shown expanding.">
        <figcaption class="fig-head">
          <span class="annot">Corridor network &mdash; route plan</span>
    ...
    case 'sumo': return `
      <figure class="fig" data-sim="sumo" data-alt="Two robots in a circular ring. The match opens autonomous, then control transfers to the drivers on the referee's signal.">
        <figcaption class="fig-head">
          <span class="annot">Ring &mdash; contact model, warning band at rim</span>
    ...
    case 'confusion': return `
      <figure class="fig" data-sim="confusion">
        <figcaption class="fig-head">
          <span class="annot">Confusion matrix &mdash; ${EVENTS.find((e) => e.slug === 'the-black-box').testSetSize} unseen test images</span>
    ...
    case 'complexity': return `
      <figure class="fig" data-sim="complexity" data-alt="Growth curves for logarithmic, linear, linearithmic, quadratic and exponential algorithms plotted against input size on a logarithmic operations axis.">
        <figcaption class="fig-head">
          <span class="annot">Growth against input size &mdash; operations, log scale</span>
    ...
    default: return `
      <figure class="fig">
        <figcaption class="fig-head"><span class="annot">Plan view &mdash; schematic</span></figcaption>
...
function pageIndex() {
  ...
  const body = `
    <section class="hero">
      <div>
        <p class="hero-kicker">A technical championship for member schools of the Indian Public Schools&rsquo; Conference, hosted by ${esc(CONFIG.venue.school)}.</p>
        <h1 class="hero-title">
          <span class="l1">IPSC</span>
          <span class="l2">IT Fest</span>
          <span class="l3">2026</span>
        </h1>
        <p class="lede hero-lede">Eight events over three days &mdash; autonomous rovers mapping ground they have never seen, sorting robots working to rules that change each round, sumo bots, a cross-school hackathon, and a dataset built to fail until you diagnose it.</p>
    ...
    <section class="band band--tight field">
      ...
      <div class="cd" data-countdown-units="${CONFIG.dates.startISO}" data-past="The fest is under way"
        ...
        <div class="cd-cell">
          <span class="cd-n" data-unit="${u}">&mdash;&mdash;</span>
          <span class="cd-k">${label}</span>
        </div>`).join('')}
      </div>
      <div class="cd-foot">
        <p class="lede" style="margin:0">Registration closes <strong>${esc(CONFIG.registration.closesLabel)}</strong> &mdash; in
          <span class="cd-inline" data-countdown="${CONFIG.registration.closesISO}" data-compact data-past="closed">&nbsp;</span>.
          Complete documentation follows in the ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}.</p>
    ...
    <section class="band">
      ...
      <p class="note">Complete documentation for each event, including detailed judging rubrics, reaches every registered school in the ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}.</p>
    </section>
      ...
      <div class="split split--even">
        ...
        <div class="est">
          ...
          <div class="est-rows">
            ...
            <div class="est-row"><label>Eligible</label><span>${esc(CONFIG.eligibility.classes)}</span></div>
            <div class="est-row" style="border-bottom:0"><label>Charges</label><span>${val(CONFIG.charges.amount)}</span></div>
          </div>
...
function pageAbout() {
  const body = phead({
    slug: 'about', sheet: '01', kicker: 'About',
    title: 'Where Technology<br>Meets Insight',
    lede: `The IPSC IT Fest brings member schools of the Indian Public Schools&rsquo; Conference to ${esc(CONFIG.venue.school)} for three days of competition across robotics, software, machine learning and enterprise.`,
  }) + `
    <section class="band split">
      <div class="prose">
        <p>A competition of this kind is usually judged on what a machine did. The run completed or it did not; the model was accurate or it was not. That standard is honest, and on the day it will decide a good deal.</p>
        <p>It is also, on its own, an increasingly poor test. A result can now be produced without being understood &mdash; a model that classifies, a function that returns, a robot that finishes its run for reasons nobody on the team could name. The output does not disclose which of these it is.</p>
        <p>The eight events were built with that difficulty in mind. The Algorithm Challenge will not accept code until the design exists on paper. The Black Box asks teams to write down where they expect their model to break before they are permitted to train it. Terra Incognita scores a map against ground the robot was never shown, so that the error a team cannot see becomes the thing they are measured on.</p>
        <p>None of this makes the result unimportant. It puts a second question beside it: not only whether the thing worked, but whether the team can say why. That is what the theme means, and it is the harder half.</p>
      </div>
      <div class="legend">
        <div class="legend-head"><p class="annot">At a glance</p></div>
        <dl>
          <dt>Host</dt><dd>${esc(CONFIG.venue.school)}</dd>
          <dt>Campus</dt><dd>${esc(CONFIG.venue.campus)}, ${esc(CONFIG.venue.city)}</dd>
          <dt>Dates</dt><dd>${esc(CONFIG.dates.label)}</dd>
          <dt>Days</dt><dd>3</dd>
          <dt>Events</dt><dd>${EVENTS.length}</dd>
          <dt>Robotics</dt><dd>${EVENTS.filter((e) => e.category === 'robotics').length}</dd>
          <dt>Theme</dt><dd><em>${esc(CONFIG.theme)}</em></dd>
          <dt>Edition</dt><dd>${val(CONFIG.edition)}</dd>
        </dl>
      </div>
    </section>

    <section class="band">
      <p class="annot">What to expect</p>
      <h2 class="h-section">Preparing a delegation</h2>
      <div class="split" style="margin-top:28px">
        <div class="prose">
          <p>Teams design and build their own machines before they travel. Robots are not provided, and are not built from scratch on campus &mdash; though the three days leave plenty of room for tuning, repair and reprogramming, and most teams will need all of it.</p>
          <p>Every participant should bring a laptop. Robotics teams should bring their own tools, spares and consumables, and should note that battery limits are set out in each event&rsquo;s specification when the documentation is released.</p>
          <p>Full rules, robot and arena specifications and detailed judging rubrics reach every registered school in the ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}. Schools are welcome to write to the committee before then with any question at all.</p>
        </div>
        <div>
          <p class="annot">The three days</p>
          <ul class="bullets" style="margin-top:14px">
            <li><strong>Monday 7 December</strong> &mdash; delegations arrive and settle into houses.</li>
            <li><strong>Tuesday 8 December</strong> &mdash; Opening Ceremony, then the first events.</li>
            <li><strong>Wednesday 9 December</strong> &mdash; competition continues across both centres.</li>
            <li><strong>Thursday 10 December</strong> &mdash; Hackathon pitches, finals, and the Closing Ceremony.</li>
          </ul>
          <div class="actions">
            <a class="btn btn-line" href="schedule.html">The full programme ${ARROW}</a>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="band field">
      <p class="annot">Host</p>
      <h2 class="h-section">${esc(CONFIG.venue.school)}</h2>
      <div class="prose" style="margin-top:20px">
        <p>${esc(CHANDBAGH.lede)}</p>
        <p>Enquiries are welcome at any time, at <a href="mailto:${esc(CONFIG.contact.email)}">${esc(CONFIG.contact.email)}</a>.</p>
      </div>
      <div class="actions">
        <a class="btn btn-fill" href="chandbagh.html">The campus ${ARROW}</a>
        <a class="btn btn-line" href="committee.html">The committee</a>
      </div>
    </section>`;
  ...
  return shell({
    ...
    description: `What the IPSC IT Fest 2026 is, who may enter, and the standard every event pack is written to.`,
    body,
  });
}
...
function pageEvent(ev, i) {
  ...
  const rubric = ev.rubric ? `
    <section class="band band--tight">
      ...
      <p class="note">Weights are those published in the brochure and the committee&rsquo;s master document. The full rubric &mdash; scoring levels, tie-break order, and who scores what &mdash; is released with the complete documentation in the ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}.</p>
    </section>` : '';
  ...
  const extras = [
    ...
    ev.classes ? `<p class="annot" style="margin-top:26px">Object classes</p>
      <ul class="bullets">${ev.classes.map((c) => `<li><strong>${esc(c.label)}</strong> &mdash; ${esc(c.note)}</li>`).join('')}</ul>` : '',
    ev.divisions ? `<p class="note"><strong>Age divisions.</strong> ${val(ev.divisions)} &mdash; ${esc(plain(ev.divisions))}</p>` : '',
    ev.seniorVersion ? `<p class="note"><strong>Age divisions.</strong> ${val(ev.seniorVersion)} &mdash; ${esc(plain(ev.seniorVersion))}</p>` : '',
    ev.companyCount ? `<p class="note"><strong>Company structure.</strong> ${val(ev.companyCount)} &mdash; ${esc(plain(ev.companyCount))}</p>` : '',
  ].filter(Boolean).join('\n      ');
    ...
    <section class="band field">
      <div class="split split--even">
        ...
        <div>
          ...
          <p class="lede" style="margin-top:16px">Full rules for ${esc(ev.brand)} &mdash; including ${ev.category === 'robotics' ? 'the robot and arena specifications' : 'the platform and environment specification'} and the detailed judging rubric &mdash; reach registered schools in the ${esc(CONFIG.documentation.releaseLabel.toLowerCase())}. Until then, do write to us with anything you would like to know.</p>
          <div class="actions">
...
function pageSchedule() {
  ...
  const body = phead({
    ...
    <section class="band band--tight">
      <p class="note">${esc(SCHEDULE_STATUS.note)}</p>
...
function pageRegistration() {
  const steps = [
    ...
    ['Choose your events', `Decide which of the eight events you are entering and who from your contingent is entering each. One student may enter a maximum of ${CONFIG.eligibility.maxEventsPerStudent} events.`],
    ...
    ['Await the documentation', `Complete rulebooks, detailed judging rubrics and charges reach registered schools in the ${CONFIG.documentation.releaseLabel.toLowerCase()}.`],
    ...
    <section class="band split">
      ...
      <div class="legend">
        ...
        <dl>
          ...
          <dt>Contingent</dt><dd>${esc(CONFIG.eligibility.delegation)}</dd>
          ...
          <dt>Charges</dt><dd>${val(CONFIG.charges.amount)}</dd>
          <dt>Boarding</dt><dd>${val(CONFIG.charges.boarding)}</dd>
...
function pageResources() {
    ...
    return `
        <article class="res">
          <div>
            ...
            ${live ? `<a class="res-t" href="${esc(r.href)}" target="_blank" rel="noopener">${esc(r.title)}</a>`
                   : `<p class="res-t">${esc(r.title)}</p>`}
            <p class="res-d">${esc(r.note)}${isTbc(r.href) ? ` ${val(r.href)}` : ''}</p>
          </div>
  ...
  const body = phead({
    ...
    title: 'Documents',
    lede: `Everything circulated so far, and everything still to come. Full documentation for all eight events reaches registered schools in the <strong>${esc(CONFIG.documentation.releaseLabel.toLowerCase())}</strong>.`,
  }) + `
    <section class="band band--tight">
      <div class="res-list">${rows}</div>
      <p class="note">Each event has its own set of documents &mdash; rules, specifications, judging rubric, safety requirements and preparation material. They are listed in full on each event&rsquo;s page.</p>
    </section>`;

  return shell({
    slug: 'resources',
    title: 'Resources',
    description: 'Brochure, invitation, registration form and the complete event documentation for the IPSC IT Fest 2026.',
    body,
  });
}
...
function pageChandbagh() {
  ...
  const travel = CHANDBAGH.travel.map((t) => `
        <div>
          ...
          <p class="card-d" style="margin-top:8px">${val(t.detail)} &mdash; ${esc(plain(t.detail))}</p>
        </div>`).join('');
  ...
  const body = phead({
    ...
    <section class="band split">
      <div class="prose">
        <p>${val(CHANDBAGH.history)}${isTbc(CHANDBAGH.history) ? ` &mdash; ${esc(plain(CHANDBAGH.history))}` : ''}</p>
        <p>${val(CHANDBAGH.life)}${isTbc(CHANDBAGH.life) ? ` &mdash; ${esc(plain(CHANDBAGH.life))}` : ''}</p>
...
function pageFaq() {
  ...
  const body = phead({
    ...
    lede: 'Answers drawn from the brochure and the rules circulated to Heads of School. Where a question is not answered here, write to the committee.',
  }) + `
    <section class="band band--tight">
      ${groups}
...
function buildIndex() {
  ...
  idx('Home', 'index.html', { g: 'Pages', r: '', top: true });
...
async function build() {
  ...
  /* Copy assets, recursing one level so assets/people comes across. */
  let portraits = 0;
  for (const e of await readdir(join(ROOT, 'assets'), { withFileTypes: true })) {
    if (e.isDirectory()) {
      await mkdir(join(OUT, 'assets', e.name), { recursive: true });
      for (const f of await readdir(join(ROOT, 'assets', e.name))) {
        await copyFile(join(ROOT, 'assets', e.name, f), join(OUT, 'assets', e.name, f));
        if (e.name === 'people' && /\.(jpe?g|png|webp)$/i.test(f)) portraits++;
      }
    } else {
      await copyFile(join(ROOT, 'assets', e.name), join(OUT, 'assets', e.name));
    }
  }
  ...
  const have = new Set((await readdir(join(ROOT, 'assets', 'people')))
  ...
  console.log(`  portraits ${portraits}/${slugs.length}\n`);

  if (missing.length) {
    console.log(`  ${missing.length} portraits missing — drop JPEGs into assets/people/:\n`);
    missing.forEach((s) => console.log(`      assets/people/${s}.jpg      ${plain(PEOPLE[s].name)}`));
    console.log('');
  }
